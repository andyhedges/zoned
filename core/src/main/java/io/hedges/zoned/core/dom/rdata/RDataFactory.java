...
public class RDataFactory {
    ...
    public static RDataDom fromBytes(DnsRecordTypeDom type, byte[] bytes, NameResolver resolver) {
        return switch (type) {
            ...
            case HTTPS -> HttpsRecordDataDom.from(bytes, resolver);
            case IPSECKEY -> IpseckeyRecordDataDom.from(bytes, resolver);
            ...
            case NAPTR -> NaptrRecordDataDom.from(bytes, resolver);
            case NS -> NsRecordDataDom.from(bytes, resolver);
            case NSEC -> NsecRecordDataDom.from(bytes, resolver);
            ...
            case RRSIG -> RrsigRecordDataDom.from(bytes, resolver);
            case SIG -> SigRecordDataDom.from(bytes, resolver);
